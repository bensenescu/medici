 ...
 */
async function getAllPools(userId: string) {
  const userMemberships = await PoolMembershipRepository.findAllByUser(userId);
  ...
  return userMemberships.map((membership) => ({
    ...membership.pool,
    memberships: (membershipsByPool.get(membership.poolId) ?? []).map((m) => ({
      ...m,
      user: m.user,
    })),
  }));
 ...
 */
async function getPool(userId: string, poolId: string) {
 ...
 */
async function createPool(userId: string, input: CreatePoolInput) {
 ...
 */
async function updatePool(userId: string, input: UpdatePoolInput) {
 ...
 * Verifies user is admin.
 */
async function deletePool(userId: string, poolId: string) {
 ...
 * Verifies user is admin.
 */
async function addMemberToPool(
  userId: string,
  poolId: string,
  friendId: string,
) {
 ...
 * Verifies user is admin.
 */
async function removeMemberFromPool(
  userId: string,
  poolId: string,
  memberId: string,
) {
  ...
  if (memberId === userId) {
    const adminCount = await PoolMembershipRepository.countAdminsByPool(poolId);
    if (adminCount === 1) {
 ...
 * Verifies user is a member.
 */
async function getPoolBalances(userId: string, poolId: string) {
  await verifyPoolMembership(userId, poolId);
